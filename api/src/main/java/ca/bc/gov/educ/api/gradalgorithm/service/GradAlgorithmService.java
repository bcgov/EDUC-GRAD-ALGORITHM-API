...
public class GradAlgorithmService {
    ...
    public GraduationData graduateStudent(UUID studentID, String gradProgram, boolean projected, String hypotheticalGradYear, String accessToken) {
        ...
        if (algorithmDataParallelDTO != null) {
            setCourseAssessmentDataForAlgorithm(algorithmDataParallelDTO.courseAlgorithmData(), algorithmDataParallelDTO.assessmentAlgorithmData(), ruleProcessorData, hypotheticalGradYear);
        }
        //Set Projected flag
        ...
        for (Map.Entry<String, OptionalProgramRuleProcessor> entry : mapOption.entrySet()) {
            ...
            getListOfOptionalProgramStatus(optionalProgramCode, obj, optionalProgramStatusList, ruleProcessorData, existingOptionalNonGradReasons);
        }
        ...
        ExistingDataSupport existingDataSupport = ExistingDataSupport.builder().existingProgramCompletionDate(existingProgramCompletionDate).existingGradMessage(existingGradMessage).gradProgram(gradProgram).optionalNonGradReasons(existingOptionalNonGradReasons).build();
    ...
    /******************************************************************************************************************
     Utility Methods
     *******************************************************************************************************************/
    private void getListOfOptionalProgramStatus(String optionalProgramCode, OptionalProgramRuleProcessor obj, List<GradAlgorithmOptionalStudentProgram> optionalProgramStatusList, RuleProcessorData ruleProcessorData, List<GradRequirement> existingOptionalNonGradReasons) {
    ...
    private void processGraduation(GradAlgorithmOptionalStudentProgram gradStudentOptionalAlg, RuleProcessorData ruleProcessorData) {
        ...
        if (gradStudentOptionalAlg.isOptionalGraduated() && ruleProcessorData.isGraduated()) {
            ...
            for (Map.Entry<String, OptionalProgramRuleProcessor> entry : ruleProcessorData.getMapOptional().entrySet()) {
                OptionalProgramRuleProcessor studentOptionalProgramsAttempted = entry.getValue();
    ...
    private String getGradDate(List<StudentCourse> studentCourses, List<StudentAssessment> studentAssessments) {
        ...
        studentCourses = studentCourses.stream().filter(StudentCourse::isUsed).collect(Collectors.toList());

        studentAssessments = studentAssessments.stream().filter(StudentAssessment::isUsed).filter(sa -> "E".compareTo(sa.getSpecialCase()) != 0).collect(Collectors.toList());
    ...
    private String getGPA(List<StudentCourse> studentCourseList, List<LetterGrade> letterGradesList) {
        ...
        float totalCredits = studentCourseList.stream().filter(sc -> sc.isUsed() && !"RM".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()) && !"SG".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()) && !"TS".equalsIgnoreCase(sc.getCompletedCourseLetterGrade())).mapToInt(StudentCourse::getCreditsUsedForGrad).sum();
        ...
        for (StudentCourse sc : studentCourseList) {
            ...
            LetterGrade letterGrade = letterGradesList.stream().filter(lg -> lg.getGrade().compareToIgnoreCase(completedCourseGrade) == 0).findFirst().orElse(null);

            if (letterGrade != null) {
                tempGpaMV = letterGrade.getGpaMarkValue();
            } else {
                if ("RM".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()) || "SG".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()) || "TS".equalsIgnoreCase(sc.getCompletedCourseLetterGrade())) {
                    tempGpaMV = "0";
                }
            }
            ...
            logger.debug("[{}/{}] Letter Grade: {} | GPA Mark Value: {} | Credits Used For GRAD: {} | Acquired Credits: {} | Total Credits: {}", sc.getCourseCode(), sc.getCourseLevel(), letterGrade, gpaMarkValue, sc.getCreditsUsedForGrad(), acquiredCredits, totalCredits);
        }
    ...
    private String getHonoursFlag(String gPA) {
        ...
        if (Float.parseFloat(gPA) > 3) return "Y";
        else return "N";
    }
    ...
    private void setCourseAssessmentDataForAlgorithm(CourseAlgorithmData courseAlgData, AssessmentAlgorithmData assessmentAlgData, RuleProcessorData ruleProcessorData, String hypotheticalGradYear) {
        ...
        sortCoursesBasedOnProgram(ruleProcessorData.getGradStatus().getProgram(), ruleProcessorData.getStudentCourses() != null ? ruleProcessorData.getStudentCourses() : new ArrayList<>(), ruleProcessorData.getStudentAssessments() != null ? ruleProcessorData.getStudentAssessments() : new ArrayList<>(), ruleProcessorData.getGradStatus().getAdultStartDate());
    }
    ...
    private void sortCoursesBasedOnProgram(String program, List<StudentCourse> studentCourses, List<StudentAssessment> studentAssessments, Date adultStartDate) {
        switch (program) {
            case "2018-EN", "2023-EN" -> {
                studentCourses.sort(new StudentCoursesComparator(program));
                studentAssessments.sort(Comparator.comparing(StudentAssessment::getProficiencyScore, Comparator.nullsLast(Double::compareTo)).reversed().thenComparing(StudentAssessment::getSpecialCase, Comparator.nullsLast(String::compareTo)).thenComparing(StudentAssessment::getSessionDate));
            }
            case "2018-PF", "2023-PF", "2004-EN", "2004-PF" ->
                    studentCourses.sort(new StudentCoursesComparator(program));
            case "1950" -> {
                /*
                ...
                if (!coursesAfterStartDate.isEmpty()) {
                    coursesAfterStartDate.sort(Comparator.comparing(StudentCourse::getCourseLevel).thenComparing(StudentCourse::getCompletedCourseLetterGrade, Comparator.nullsLast(String::compareTo)).thenComparing(StudentCourse::getCompletedCoursePercentage, Comparator.reverseOrder()));
                    studentCourses.addAll(coursesAfterStartDate);
                }
                if (!coursesOnOrBeforeStartDate.isEmpty()) {
                    coursesOnOrBeforeStartDate.sort(Comparator.comparing(StudentCourse::getCourseLevel).thenComparing(StudentCourse::getCompletedCourseLetterGrade, Comparator.nullsLast(String::compareTo)).thenComparing(StudentCourse::getCompletedCoursePercentage, Comparator.reverseOrder()));
    ...
    private void setGradStatusAlgorithmResponse(String gradProgram, String existingProgramCompletionDate, GradAlgorithmGraduationStudentRecord gradStatus, boolean checkSCCPNOPROG, RuleProcessorData ruleProcessorData) {
        ...
        if ((existingProgramCompletionDate == null || ruleProcessorData.isProjected()) && gradStatus.getSchoolAtGradId() == null) {
            ...
            gradStatus.setSchoolAtGradId(ruleProcessorData.getGradStudent().getSchoolOfRecordId() != null ? UUID.fromString(ruleProcessorData.getGradStudent().getSchoolOfRecordId()) : null);
        ...
        if (checkSCCPNOPROG && gradStatus.getSchoolAtGradId() == null) {
            gradStatus.setSchoolAtGrad(ruleProcessorData.getGradStudent().getSchoolOfRecord());
            gradStatus.setSchoolAtGradId(UUID.fromString(ruleProcessorData.getGradStudent().getSchoolOfRecordId()));
        }
    }
    ...
    private Pair<Boolean, String> checkHonoursExemptRule(String program, List<StudentCourse> studentCourseList) {
        ...
        float totalCreditsTSSGRM = studentCourseList.stream().filter(sc -> sc.isUsed() && ("RM".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()) || "SG".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()) || "TS".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()))).mapToInt(StudentCourse::getCreditsUsedForGrad).sum();
        float totalCoursesTSSG = studentCourseList.stream().filter(sc -> sc.isUsed() && ("SG".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()) || "TS".equalsIgnoreCase(sc.getCompletedCourseLetterGrade()))).count();

        switch (program) {
            case "2023-EN", "2023-PF", "2018-EN", "2018-PF" -> {
                StudentCourse sCCheck = studentCourseList.stream().filter(sc -> sc.isUsed() && (sc.getCourseCode().equalsIgnoreCase("GT") || sc.getCourseCode().equalsIgnoreCase("GTF") || sc.getCourseCode().equalsIgnoreCase("PORT") || sc.getCourseCode().equalsIgnoreCase("PORTF"))).findAny().orElse(null);
                if (sCCheck == null) {
    ...
    private void processGradMessages(boolean checkSCCPNOPROG, ExistingDataSupport existingDataSupport, Map<String, OptionalProgramRuleProcessor> mapOption, RuleProcessorData ruleProcessorData, GraduationData graduationData) {
        GradMessageRequest gradMessageRequest = GradMessageRequest.builder().gradProgram(existingDataSupport.getGradProgram()).gradDate(graduationData.getGradStatus().getProgramCompletionDate()).honours(graduationData.getGradStatus().getHonoursStanding()).programName(ruleProcessorData.getGradProgram().getProgramName()).projected(ruleProcessorData.isProjected()).schoolAtGradName(graduationData.getGradStatus().getSchoolAtGradName()).existingGraduated(existingDataSupport.getExistingProgramCompletionDate() != null).existingOptionalProgramGraduated(existingDataSupport.getOptionalNonGradReasons() == null || existingDataSupport.getOptionalNonGradReasons().isEmpty()).graduated(ruleProcessorData.isGraduated()).build();
        ...
        if (checkSCCPNOPROG) {
            gradMessageRequest = GradMessageRequest.builder().gradProgram(existingDataSupport.getGradProgram()).msgType(ruleProcessorData.isGraduated() ? MSG_TYPE_GRADUATED : MSG_TYPE_NOT_GRADUATED).gradDate(graduationData.getGradStatus().getProgramCompletionDate()).honours(graduationData.getGradStatus().getHonoursStanding()).programName(ruleProcessorData.getGradProgram().getProgramName()).projected(ruleProcessorData.isProjected()).existingGraduated(existingDataSupport.getExistingProgramCompletionDate() != null).existingOptionalProgramGraduated(existingDataSupport.getOptionalNonGradReasons() == null || existingDataSupport.getOptionalNonGradReasons().isEmpty()).graduated(ruleProcessorData.isGraduated()).build();
            graduationData.setGradMessage(getGradMessages(gradMessageRequest, null, ruleProcessorData));
        }
    }

    private void createCompleteGradMessage(GradMessageRequest gradMessageRequest, StringBuilder currentGradMessage, TranscriptMessage result, Map<String, OptionalProgramRuleProcessor> mapOptional, RuleProcessorData ruleProcessorData, String opMessage) {
        ...
        if (!optPrograms.isEmpty() && opMessage.equalsIgnoreCase(GRADUATED) && (!gradMessageRequest.isProjected() || gradMessageRequest.isExistingOptionalProgramGraduated())) {
